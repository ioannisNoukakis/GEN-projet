...
import Client.pgm.GameAlerts.SearchingPlayer;
import GameManager.API;
import Protocol.MiniPersonnage;
        ...
        fight.setOnAction(event -> {
            ...
            try {
                if(api.joinFight(personnages.get(index).getId()) && api.init()) {
                    api.start();
                    Platform.runLater(() -> {
                        searchingPlayer.setAlerteVisible(false);
                    });
                    GameView.changeScene(FightView.create(api));
                } else {
                    AlertWindow.create("Cannot connect to the server (try again later).", "");
                    SendCharacterData data2 = api.connect(Connect.userName, Connect.password);
                    GameView.changeScene(Menu.create(data2, api));
                }
